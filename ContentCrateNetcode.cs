        ...
        public static void HandlePacket(Mod mod, BinaryReader reader, int whoAmI)
        {
            ...
            catch(Exception e)
            {
                if (e is EndOfStreamException eose)
                    ContentCrate.Instance.Logger.Error("Failed to parse Calamity packet: Packet was too short, missing data, or otherwise corrupt.", eose);
                else if (e is ObjectDisposedException ode)
                    ContentCrate.Instance.Logger.Error("Failed to parse Calamity packet: Packet reader disposed or destroyed.", ode);
                else if (e is IOException ioe)
                    ContentCrate.Instance.Logger.Error("Failed to parse Calamity packet: An unknown I/O error occurred.", ioe);
                else
